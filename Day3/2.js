...
function fetchAvailableMovies() {
  ...
  //   console.log(movieFetched);
    ...
    const movieList = ["Movie A", "Movie B", "Movie C", "Movie D"];
    const movieChoose = await chooseMovie(movieList);
    console.log(movieChoose);
  
    const movie="Movie A";
    const showtimeSelected = await selectShowtime(movie);
    ...
    const [EmailConf, Smsconf] = await Promise.all([
